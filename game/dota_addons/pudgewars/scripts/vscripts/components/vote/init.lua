...
ListenToGameEvent('game_rules_state_change', function(keys)
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_HERO_SELECTION then
		...
		for category, pidVoteTable in pairs(votes) do
			...
			local kills_to_win_table = {}
			kills_to_win_table[1] = 50
			kills_to_win_table[2] = 75
			kills_to_win_table[3] = 100

			-- Act on the winning vote
			if category == "gamemode" then
				PudgeWarsMode.kills_to_win = kills_to_win_table[highest_key]
			end
