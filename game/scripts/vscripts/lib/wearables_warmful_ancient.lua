...
require("lib/activity_modifier")

LinkLuaModifier("modifier_wearable", "lib/modifiers/modifier_wearable.lua", LUA_MODIFIER_MOTION_NONE)
...
function Wearable:Init()
	...
	if IsInToolsMode() then
		Wearable.items_game = {}
		Wearable.items_game.items = {}
		Wearable.items_game.colors = {}
	else
...
function Wearable:WearAfterRespawn(hUnit, hNewWears)
	...
	for sSlotName, hNewWear in pairs(hNewWears) do
		...
		if Wearable.respawn_items[sItemDef] == 1 and Wearable.m_bRespawnWear then
			-- 重生模型设置重生饰品table
			local hWear = {}
...
function Wearable:_WearItemsRespawn(hUnitOrigin, hNewWears)
	...
	if Wearable.m_bRespawnWear then
...
function Wearable:ShouldRespawnForItem(hUnit, sItemDef)
	...
	if not Wearable.m_bRespawnWear and bHasRespawnItem then
		-- 已关闭重生饰品模式，但原单位仍有重生饰品，需要重生一个默认单位
		return true
	elseif hWearOld and Wearable.m_bRespawnWear and hWearOld["bRespawnItem"] then
		-- 被替换的槽位中有重生饰品，需要重生一个不包含该重生饰品的单位
		return true
	elseif Wearable.m_bRespawnWear and Wearable.respawn_items[sItemDef] == 1 then
		-- 新饰品为重生饰品
		return true
	else
		return false
...
function Wearable:_WearProp(hUnit, sItemDef, sSlotName, sStyle)
	...
	if not hItem then
		print("Wearable:_WearProp: item not found", sItemDef)
		return
	end
